            ...
            <div class="bg-light border shadow-sm d-flex flex-row align-items-center">
                ...
                {{-- main内容 --}}
                    <div class="text-center">
                        <form action="/product_regis" method="POST">
                            @csrf
                            <h1 class="regis">商品登録</h1>
                                <input class="formsize" type="text"  name="name" placeholder="名前" maxlength="15" required autofocus><br>
                                
                                <select class="formsize" name="type_id" id="">
                                    @foreach($types as $type)
                                    <option placeholder="種別" value="{{ $type->id}}">{{ $type->type_name}}</option>
                                    @endforeach
                                </select><br>
                                  <input class="formsize"  type="text"  name="detail" placeholder="詳細" maxlength="254" required><br>
