    ...
    <div class="d-flex flex-row w-100" style="height: calc(100% - 50px)">
        ...
        <nav class="bg-dark n_home">
            ...
            <ul class="nav flex-column m-0 p-3">
                ...
                <li class="nav-item mb-2"><a href="/type" class="nav-link">商品登録</a></li>
            </ul>
        </nav>
        {{-- メインコンテンツ --}}
        <main class="w-100 bg-light m_home">
            ...
            <div class="bg-light border shadow-sm d-flex flex-row align-items-center">
                ...
                <form action="/edit_complete"  method="POST">
                    @csrf
                    <h1 class="regis">会員編集</h1>
                        <input class="formsize" type="hidden"  name="id" value="{{ $items->id }}"><br>
                        <input class="formsize" type="text"  name="name"  maxlength="10" value="{{ $items->name }}" required autofocus><br>
                        <select class="formsize" name="type_id" id="">
                            {{-- @foreach($types as $type)
                            <option value="{{ $type->id}}">{{ $type->type_name}}</option>
                            @endforeach --}}
                            @foreach($types as $type)
                            <option value="{{$type->id}}" 
                             @if($type->id==$items->type_id) selected @endif>
                            {{ $type->type_name }}</option>
                       @endforeach
                        </select>
                        <br>
                        <input class="formsize" type="text"  name="detail"  maxlength="254" value="{{ $items->detail }}" required><br>
                        <button class="btn1" type="submit">編集</button>
                </form>
                <form action="/delete" method="POST">
                    @csrf
                    <input class="formsize" type="hidden"  name="id" value="{{ $items->id }}">
                    <button class="btn1" type="submit">削除</button>
                </form>
                </div>    
                </main>
            </body>
